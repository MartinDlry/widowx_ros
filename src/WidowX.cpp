...
WidowX::WidowX()
{
    pWidowXServos = new WidowXServos();
    mLengths = { L0 , L1 , L2 };
}
...
Vec3 WidowX::headPosition()
{
    ...
    v2.mX = - v2.mX;
    double a = pWidowXServos-> mServos[0]->radian();
...
{ 
    ...
    for( int i = 1 ; i < 4 ; i++ )
    {
        a += pWidowXServos-> mServos[i]->radian(); //add the next servo relative radian angle
...
bool WidowX::canReachGoal()
{
    ...
    if ( Circle( 0 , 0 , mLengths[0] ).intersect( Circle( wrist , mLengths[1] ) , &intersections) )
    {
        ...
        int variation1 = pWidowXServos->variation( &testServos );
        ...
        {
            if( b &&  pWidowXServos->variation( &testServos ) > variation1 ) //if both are valid but first is better
                mServosGoalPositions = pos1;
            else
                mServosGoalPositions = (testServos.getPositions()); 
            computeServosGoalSpeeds();
            return true;
        }
        else if( b )
        {
...
void WidowX::goToGoal()
{
    if( canReachGoal())
    {
        pWidowXServos->setPositions( mServosGoalPositions );
        pWidowXServos->setSpeeds( mServosGoalSpeeds );
    }
}
...
void WidowX::computeServosGoalSpeeds()
{
    std::array<int,6> pos = pWidowXServos->getPositions();
    int max = 0;
    for( int i = 0 ; i < 6 ; i ++)
    {
        mServosGoalSpeeds[i] = abs( pos[i] - mServosGoalPositions[i]);
        if( mServosGoalSpeeds[i] > max )
            max = mServosGoalSpeeds[i];
    }
    max++;
    double ratio = 100.0/max;
    for( int i = 0 ; i < 6 ; i ++)
        mServosGoalSpeeds[i] = mServosGoalSpeeds[i] * ratio +1;
}
